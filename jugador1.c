...
void convertirADama(Nodo2 * nodo, int i, int j){ //convierte un peon a dama
...
int fichaEnCasillero(Nodo2 *n, int i, int j){ //me dice que ficha se encuentra en el casillero i j
...
int asignarRiesgoSiPuedoComer(Nodo2 * n, int i,int j, int jug){ //En caso de poder comer un peon le resta 1 a riesgo, en caso de dama 2
  int riesgo = 0, aux = 1;
...
bool damaMePuedeComer(Nodo2 *n, int i, int j, int jug){ // si una dama me puede comer me devuelve true
...
int evaluarRiesgo(Nodo2 *t, int i, int j, int jug){ //devuelve el riesgo basado en comparaciones despues de haber realizado el movimiento
  int riesgo = 0;
...
int evaluarRiesgoInicial(Nodo2 *t, int i, int j, int jug){ //devuelve el riesgo basado en comparaciones despues de haber realizado el movimiento
  int riesgo = 0;
  if(estoyEnBorde(i,j) || ((fichaAliada(t,i+filaAvance(jug),j+1,jug)) && (fichaAliada(t,i+filaAvance(jug),j-1,jug)))) {//si tengo dos fichas atras una a cada lado entonces 0
    riesgo += 0;
  } else{
...
Array * agregarMovimientosDama(Nodo2 *n, int i, int j, int jug){//devuelve un arreglo con todos los movimientos posibles para la dama posicionada en i,j
  Nodo2 *nuevo;
  Array *movimientos = crearArray(0);
  int aux = 1;
  bool salir1, salir2, salir3, salir4;
...
Array* recursionComiendo(Nodo2 *t, int jugador, int i, int j) {//i es para filas y j para columnas, sory reutilice un toque de codigo paja cambiarlo
  bool nomovDerecha, nomovIzquierda;
  int iDestino,jDestino;
  Nodo2 *nuevo;
  Array *movimientos = crearArray(0);
